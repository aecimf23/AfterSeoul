...
public static class OrientationSimulation
{
    ...
    public static void Main(string[] args)
    {
        ...
        var rows=new List<string>{"employer,profile,startVariant,stepSeconds,score,crafts,steps,firstSaleSec,hireSec,equipSec,departSec,returnSec,firstQuestSec,delivered,cash30,level30,injured,lost,firstReturnBeyond30,orientationDeliverySec"};
        ...
        for(int sample=0;sample<12;sample++)
        {
            ...
            for(int t=0;t<=1800;t+=5)
            {
                ...
                {
                    var exp=s.DepartOrientation(new[]{scavId});
                    if(exp!=null)
                    {
                ...
                {
                    returnSeen=true;returned=t;log(t,"return; cash="+s.Save.Player.Money); Check(s.DeliverOrientation(),"orientation delivery"); log(t,"orientation delivered +50000");
                }
                if(profile!=3)
                {
            ...
            rows.Add(string.Join(",",employer.NpcId,name,sample,stepSeconds,score,crafts,steps,firstSale,hire,equip,depart,returned,firstQuest,delivered,s.Save.Player.Money,s.Save.Player.Level,
                ...
                expedition==null ? -1 : (int)(expedition.ReturnsAt-start).TotalSeconds, s.Save.Orientation.Stage==OrientationStage.Completed ? returned : -1));
