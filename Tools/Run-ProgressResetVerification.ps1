...
$out=Join-Path $root 'Library/ProgressResetVerification'
...
& $mono $compiler /nologo /nowarn:0649 "/out:$out/ProgressResetVerification.exe" "/r:$out/Newtonsoft.Json.dll" "/r:$netstandard" $sources "$PSScriptRoot/ProgressResetVerification.cs"
if($LASTEXITCODE -ne 0){throw 'Compilation failed'}
& $mono "$out/ProgressResetVerification.exe" "$root/Assets/StreamingAssets/Data"
if($LASTEXITCODE -ne 0){throw 'Progress reset verification failed'}
